...
def registration():
    ...
    if request.method == 'POST':
        ...
        try:
            # 获取最小可用编号
            next_id = get_next_available_id(cursor, 'register', 'r_num')
            # 开启IDENTITY_INSERT以允许显式插入ID
            cursor.execute("SET IDENTITY_INSERT register ON")
            cursor.execute("INSERT INTO register (r_num, r_patient_id, r_P_name, r_sex, r_dept, r_doctor_id, r_name) VALUES (?, ?, ?, ?, ?, ?, ?)",
                           (next_id, r_patient_id, r_P_name, r_sex, r_dept, r_doctor_id, r_name))
            cursor.execute("SET IDENTITY_INSERT register OFF")
            conn.commit()
    ...
    cursor.execute("""
        ...
        FROM register WHERE is_delete = 0 ORDER BY create_time DESC
    """)
    registrations = cursor.fetchall()
    
    # 获取科室列表（从医生表中去重）
    cursor.execute("SELECT DISTINCT d_dept FROM doctor WHERE is_delete = 0 ORDER BY d_dept")
    departments = [row[0] for row in cursor.fetchall()]
    
    # 获取所有在岗医生
    cursor.execute("SELECT d_octor_id, d_name, d_dept FROM doctor WHERE is_delete = 0 AND is_jobing = 1 ORDER BY d_dept, d_name")
...
def delete_registration(id):
    ...
    try:
        ...
        cursor.execute("SELECT id FROM recipel WHERE registration_id = ?", (id,))
        ...
        if presc_ids:
            ...
            # 2. 级联删除取药票单
            cursor.execute(f"DELETE FROM PGM WHERE t_id IN ({placeholders})", presc_ids_str)
            
            # 3. 级联删除支付记录
            cursor.execute(f"DELETE FROM pay WHERE t_id IN ({placeholders})", presc_ids_str)
            
            # 4. 级联删除收费记录
            cursor.execute(f"DELETE FROM charge WHERE toll_id IN ({placeholders})", presc_ids_str)
            
            # 5. 恢复药品库存 & 删除处方药品记录
            for pid in presc_ids:
                # 恢复库存
                cursor.execute("SELECT drug_id, quantity FROM prescription_drug WHERE prescription_id = ?", (pid,))
                p_drugs = cursor.fetchall()
                for pd in p_drugs:
                     cursor.execute("UPDATE drugs SET drug_quantity = drug_quantity + ? WHERE drug_id = ?", (pd[1], pd[0]))
                
                # 删除处方药品
                cursor.execute("DELETE FROM prescription_drug WHERE prescription_id = ?", (pid,))

            # 6. 删除处方
            cursor.execute(f"DELETE FROM recipel WHERE id IN ({placeholders})", presc_ids)
            
        # 7. 删除挂号记录
        cursor.execute("DELETE FROM register WHERE r_num = ?", (id,))
...
def get_patient_info(patient_id):
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("SELECT p_atient_id, p_name, p_sex FROM patient WHERE p_atient_id = ? AND is_delete = 0", (patient_id,))
        result = cursor.fetchone()
        conn.close()
...
def doctors():
    ...
    if request.method == 'POST':
        ...
        try:
            cursor.execute("INSERT INTO doctor (d_octor_id, d_name, d_sex, d_age, d_dept, d_tel) VALUES (?, ?, ?, ?, ?, ?)",
    ...
    cursor.execute("SELECT * FROM doctor WHERE is_delete = 0")
    doctors = cursor.fetchall()
    conn.close()
...
def delete_doctor(id):
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("DELETE FROM doctor WHERE d_octor_id = ?", (id,))
        conn.commit()
...
def edit_doctor_submit():
    ...
    try:
        ...
        cursor.execute("""
            UPDATE doctor SET d_name = ?, d_sex = ?, d_age = ?, d_dept = ?, d_tel = ?, update_time = GETDATE()
            WHERE d_octor_id = ?
...
def patients():
    ...
    if request.method == 'POST':
        ...
        try:
            cursor.execute("INSERT INTO patient (p_atient_id, p_name, p_age, p_sex, p_tel, p_inf) VALUES (?, ?, ?, ?, ?, ?)",
    ...
    cursor.execute("SELECT * FROM patient WHERE is_delete = 0")
...
def delete_patient(id):
    ...
    try:
        ...
        cursor.execute("SELECT p_name FROM patient WHERE p_atient_id = ?", (id,))
        res = cursor.fetchone()
        if not res:
            flash('病人不存在', 'danger')
            return redirect(url_for('patients'))
        ...
        cursor.execute("SELECT toll_id FROM charge WHERE patient_id = ?", (id,))
        ...
        if toll_ids:
            ...
            sql = f"DELETE FROM PGM WHERE t_id IN ({placeholders})"
            cursor.execute(sql, toll_ids)

        # 2. 级联删除支付记录
        cursor.execute("DELETE FROM pay WHERE patient_id = ?", (id,))

        # 3. 级联删除收费记录
        cursor.execute("DELETE FROM charge WHERE patient_id = ?", (id,))

        # 4. 处理处方（恢复库存 + 删除）
        # 按病人姓名查找处方（处方表使用病人姓名）
        cursor.execute("SELECT id FROM recipel WHERE patient_name = ?", (p_name,))
        prescriptions = cursor.fetchall()
        
        for presc in prescriptions:
            presc_id = presc[0]
            # 恢复库存
            cursor.execute("SELECT drug_id, quantity FROM prescription_drug WHERE prescription_id = ?", (presc_id,))
            p_drugs = cursor.fetchall()
            for pd in p_drugs:
                cursor.execute("UPDATE drugs SET drug_quantity = drug_quantity + ? WHERE drug_id = ?", (pd[1], pd[0]))
            
            # 删除处方药品详情
            cursor.execute("DELETE FROM prescription_drug WHERE prescription_id = ?", (presc_id,))
        
        # 删除处方记录
        cursor.execute("DELETE FROM recipel WHERE patient_name = ?", (p_name,))

        # 5. 删除挂号记录
        cursor.execute("DELETE FROM register WHERE r_patient_id = ?", (id,))

        # 6. 删除病人记录
        cursor.execute("DELETE FROM patient WHERE p_atient_id = ?", (id,))
...
def delete_patient_medical_info(id):
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("UPDATE patient SET p_inf = '' WHERE p_atient_id = ?", (id,))
        conn.commit()
...
def update_patient_medical_info(id):
    ...
    try:
        ...
        cursor.execute("UPDATE patient SET p_inf = ? WHERE p_atient_id = ?", (p_inf, id))
        conn.commit()
        conn.close()
...
def edit_patient_submit():
    ...
    try:
        ...
        cursor.execute("""
            UPDATE patient SET p_name = ?, p_age = ?, p_sex = ?, p_tel = ?, p_inf = ?, update_time = GETDATE()
            WHERE p_atient_id = ?
...
def drugs():
    ...
    if request.method == 'POST':
        ...
        try:
            cursor.execute("INSERT INTO drugs (drug_id, drug_name, drug_price, drug_quantity, drug_storage, drug_date, usefull_life) VALUES (?, ?, ?, ?, ?, ?, ?)",
...
def delete_drug(id):
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("DELETE FROM drugs WHERE drug_id = ?", (id,))
        conn.commit()
...
def edit_drug_submit():
    ...
    try:
        ...
        cursor.execute("""
            UPDATE drugs SET drug_name = ?, drug_price = ?, drug_quantity = ?, drug_storage = ?, update_time = GETDATE()
            WHERE drug_id = ?
...
def prescriptions():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            cursor.execute("SELECT id FROM recipel WHERE registration_id = ?", (registration_id,))
            ...
            cursor.execute("SELECT r_doctor_id FROM register WHERE r_num = ? AND is_delete = 0", (registration_id,))
            ...
            # 获取最小可用的处方编号
            prescription_id = get_next_available_id(cursor, 'recipel', 'id')
            
            # 插入处方主记录（包含挂号编号）
            cursor.execute("SET IDENTITY_INSERT recipel ON")
            cursor.execute("INSERT INTO recipel (id, doctor_id, patient_name, registration_id) VALUES (?, ?, ?, ?)",
                           (prescription_id, doctor_id, patient_name, registration_id))
            cursor.execute("SET IDENTITY_INSERT recipel OFF")
            ...
            for i, drug_id in enumerate(drug_ids):
                if drug_id and i < len(counts) and counts[i]:
                    ...
                    # 检查库存是否充足
                    cursor.execute("SELECT drug_quantity, drug_name, drug_price FROM drugs WHERE drug_id = ? AND is_delete = 0", (drug_id,))
                    ...
                    # 插入处方药品记录
                    cursor.execute("INSERT INTO prescription_drug (prescription_id, drug_id, quantity) VALUES (?, ?, ?)",
                                   (prescription_id, drug_id, quantity))
                    
                    # 扣减药品库存
                    cursor.execute("UPDATE drugs SET drug_quantity = drug_quantity - ? WHERE drug_id = ?",
                                   (quantity, drug_id))
                    
                    # 获取病人ID用于收费
                    cursor.execute("SELECT p_atient_id FROM patient WHERE p_name = ? AND is_delete = 0", (patient_name,))
                    ...
                    cursor.execute("INSERT INTO charge (toll_id, t_name, patient_id, drug_id, drug_quantity, amount) VALUES (?, ?, ?, ?, ?, ?)",
    ...
    # 获取药品列表（有库存的）
    cursor.execute("SELECT drug_id, drug_name, drug_price, drug_quantity FROM drugs WHERE is_delete = 0 AND drug_quantity > 0")
    drugs = cursor.fetchall()
...
def delete_prescription(id):
    ...
    try:
        ...
        # 3. 删除关联的取药票单（如果有）
        cursor.execute("DELETE FROM PGM WHERE t_id = ?", (str(id),))

        # 4. 删除关联的支付记录（如果有）
        # 需要先找到病人ID以便正确删除支付记录（如需要），但支付表主键是(patient_id, t_id)。
        # 经检查架构，支付表有 t_id。DELETE FROM pay WHERE t_id = ? 应该可行，因为 t_id 在每次处方收费中是唯一的。
        cursor.execute("DELETE FROM pay WHERE t_id = ?", (str(id),))

        # 5. 删除关联的收费记录
        cursor.execute("DELETE FROM charge WHERE toll_id = ?", (str(id),))
        
        # 4. 级联删除处方药品记录
        cursor.execute("DELETE FROM prescription_drug WHERE prescription_id = ?", (id,))
        
        # 5. 删除处方
        cursor.execute("DELETE FROM recipel WHERE id = ?", (id,))
...
def payments():
    ...
    if request.method == 'POST':
        ...
        try:
            # 插入支付记录
            cursor.execute("INSERT INTO pay (patient_id, t_id, price) VALUES (?, ?, ?)",
            ...
            for item in charge_items:
                ...
                amount = item[2]
                # 检查是否已存在取药票单
                cursor.execute("SELECT 1 FROM PGM WHERE t_id = ? AND drug_id = ?", (t_id, drug_id))
                if not cursor.fetchone():
                    cursor.execute("INSERT INTO PGM (t_id, drug_id, quantity, price) VALUES (?, ?, ?, ?)",
    ...
    cursor.execute("""
        ...
        FROM pay p
        LEFT JOIN patient pt ON p.patient_id = pt.p_atient_id
        WHERE p.is_delete = 0
...
def delete_payment(patient_id, t_id):
    ...
    try:
        # 级联删除取药票单
        cursor.execute("DELETE FROM PGM WHERE t_id = ?", (t_id,))
        # 删除支付记录
        cursor.execute("DELETE FROM pay WHERE patient_id = ? AND t_id = ?", (patient_id, t_id))
        conn.commit()
...
def pickups():
    ...
    cursor.execute("""
        ...
        FROM PGM WHERE is_delete = 0 ORDER BY create_time DESC
    """)
...
def mark_pickup(t_id, drug_id):
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("UPDATE PGM SET is_picked = 1, update_time = GETDATE() WHERE t_id = ? AND drug_id = ?", (t_id, drug_id))
        conn.commit()
...
def statistics():
    ...
    try:
        # 基础统计
        cursor.execute("SELECT COUNT(*) FROM patient WHERE is_delete = 0")
        stats['total_patients'] = cursor.fetchone()[0]
        
        cursor.execute("SELECT COUNT(*) FROM doctor WHERE is_delete = 0 AND is_jobing = 1")
        stats['total_doctors'] = cursor.fetchone()[0]
        
        cursor.execute("SELECT COUNT(*) FROM drugs WHERE is_delete = 0")
        stats['total_drugs'] = cursor.fetchone()[0]
        
        # 挂号统计
        cursor.execute("SELECT COUNT(*) FROM register WHERE is_delete = 0")
        stats['total_registrations'] = cursor.fetchone()[0]
        
        cursor.execute("SELECT COUNT(*) FROM register WHERE is_delete = 0 AND CAST(create_time AS DATE) = CAST(GETDATE() AS DATE)")
        stats['today_registrations'] = cursor.fetchone()[0]
        
        cursor.execute("SELECT COUNT(*) FROM register WHERE is_delete = 0 AND create_time >= DATEADD(day, -7, GETDATE())")
        stats['week_registrations'] = cursor.fetchone()[0]
        
        cursor.execute("SELECT COUNT(*) FROM register WHERE is_delete = 0 AND MONTH(create_time) = MONTH(GETDATE()) AND YEAR(create_time) = YEAR(GETDATE())")
        stats['month_registrations'] = cursor.fetchone()[0]
        
        # 处方统计
        cursor.execute("SELECT COUNT(*) FROM recipel WHERE is_delete = 0")
        stats['total_prescriptions'] = cursor.fetchone()[0]
        
        cursor.execute("SELECT COUNT(*) FROM recipel WHERE is_delete = 0 AND CAST(create_time AS DATE) = CAST(GETDATE() AS DATE)")
        stats['today_prescriptions'] = cursor.fetchone()[0]
        
        cursor.execute("SELECT COUNT(*) FROM recipel WHERE is_delete = 0 AND create_time >= DATEADD(day, -7, GETDATE())")
        stats['week_prescriptions'] = cursor.fetchone()[0]
        
        cursor.execute("SELECT COUNT(*) FROM recipel WHERE is_delete = 0 AND MONTH(create_time) = MONTH(GETDATE()) AND YEAR(create_time) = YEAR(GETDATE())")
        stats['month_prescriptions'] = cursor.fetchone()[0]
        
        # 收入统计
        cursor.execute("SELECT ISNULL(SUM(price), 0) FROM pay WHERE is_delete = 0")
        stats['total_revenue'] = cursor.fetchone()[0] or 0
        
        cursor.execute("SELECT ISNULL(SUM(price), 0) FROM pay WHERE is_delete = 0 AND CAST(create_time AS DATE) = CAST(GETDATE() AS DATE)")
        stats['today_revenue'] = cursor.fetchone()[0] or 0
        
        cursor.execute("SELECT ISNULL(SUM(price), 0) FROM pay WHERE is_delete = 0 AND create_time >= DATEADD(day, -7, GETDATE())")
        stats['week_revenue'] = cursor.fetchone()[0] or 0
        
        cursor.execute("SELECT ISNULL(SUM(price), 0) FROM pay WHERE is_delete = 0 AND MONTH(create_time) = MONTH(GETDATE()) AND YEAR(create_time) = YEAR(GETDATE())")
        stats['month_revenue'] = cursor.fetchone()[0] or 0
        
        # 取药统计
        cursor.execute("SELECT COUNT(*) FROM PGM WHERE is_delete = 0")
        stats['total_pickups'] = cursor.fetchone()[0] or 0
        
        cursor.execute("SELECT COUNT(*) FROM PGM WHERE is_delete = 0 AND is_picked = 1")
        stats['completed_pickups'] = cursor.fetchone()[0] or 0
        
        cursor.execute("SELECT COUNT(*) FROM PGM WHERE is_delete = 0 AND (is_picked = 0 OR is_picked IS NULL)")
        ...
        cursor.execute("""
            SELECT COUNT(DISTINCT c.toll_id) FROM charge c
            WHERE c.is_delete = 0
            AND NOT EXISTS (SELECT 1 FROM pay WHERE patient_id = c.patient_id AND t_id = c.toll_id AND is_delete = 0)
        """)
        ...
        # 库存预警（低于10个）
        cursor.execute("SELECT drug_id, drug_name, drug_quantity FROM drugs WHERE is_delete = 0 AND drug_quantity < 10 ORDER BY drug_quantity ASC")
        ...
        cursor.execute("""
            SELECT d.d_name, d.d_dept, 
                   (SELECT COUNT(*) FROM recipel r WHERE r.doctor_id = d.d_octor_id AND r.is_delete = 0) as prescription_count,
                   (SELECT COUNT(*) FROM register reg WHERE reg.r_name = d.d_name AND reg.is_delete = 0) as registration_count
            FROM doctor d
        ...
        cursor.execute("""
            ...
            FROM prescription_drug pd
            JOIN drugs dr ON pd.drug_id = dr.drug_id
