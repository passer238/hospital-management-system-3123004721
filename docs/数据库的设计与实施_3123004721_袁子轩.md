![README.md](/assets/88dfcbaf-6105-478c-b97a-f01726f9b036.md)# 数据库的设计与实施实验报告
...
选用的DBMS：Microsoft SQL Server 2022
说明：该系统旨在管理医院的日常业务，包括病人挂号、医生管理、药品库存、处方开具、费用缴纳等核心功能。采用SQL Server数据库管理系统，结合Python Flask框架和pyodbc数据库连接库进行开发。
...
本项目采用Microsoft SQL Server 2022作为数据库管理系统。
选择原因：
1） SQL Server具有企业级的稳定性和安全性
2） 良好的Windows平台集成，适合本地开发环境
3） 支持完整的ACID事务处理和高级查询优化
4） 提供强大的管理工具（SSMS）便于数据库维护

物理模型设计说明：

1. 数据库引擎：SQL Server Database Engine
...
3. 数据类型选择：
   ...
   - 主键编号：INT IDENTITY 或 VARCHAR
   - 整数型：INT, TINYINT, BIGINT
   - 小数：DECIMAL(10,2) （精确货币计算）
   - 字符串：VARCHAR/NVARCHAR（变长）
   - 日期时间：DATETIME
   - 逻辑值：TINYINT（0/1）或BIT
4. 索引设计：

   - 主键自动创建聚集索引
   - 外部标识字段创建非聚集索引（如r_patient_id, drug_id）
...
- 数据库类型：Microsoft SQL Server 2022
- 主机地址：localhost
- 端口：默认端口（动态分配）
- 数据库名：hospital_information
- 身份验证：Windows身份验证（Trusted_Connection）
- 字符集：支持Unicode
- ODBC驱动：ODBC Driver 17 for SQL Server

Python Flask应用配置（app.py）：
...
已成功连接到SQL Server数据库hospital_information，使用pyodbc库进行数据库连接。

pyodbc连接优势：

- 标准化：遵循Python DB-API 2.0规范
- 跨平台：支持Windows、Linux、macOS
- 高性能：底层使用C语言实现，效率高
- 稳定可靠：成熟的ODBC接口，兼容性好
- 连接池管理：可配合SQLAlchemy实现连接池
- 数据库操作优化：
  - 参数化查询：防止SQL注入
  - 事务支持：commit/rollback机制
  - 游标管理：支持多种游标类型
...
```sql
-- SQL Server建表语句
IF OBJECT_ID('patient_3123004721_袁子轩', 'U') IS NOT NULL
    DROP TABLE patient_3123004721_袁子轩;

CREATE TABLE patient_3123004721_袁子轩 (
  p_atient_id VARCHAR(20) NOT NULL,
  p_name VARCHAR(20) NOT NULL,
  p_age TINYINT NOT NULL,
  p_sex VARCHAR(2) NOT NULL,
  p_tel VARCHAR(20) NOT NULL,
  p_inf VARCHAR(50) NOT NULL,
  is_delete TINYINT NOT NULL DEFAULT 0,
  create_time DATETIME DEFAULT GETDATE(),
  update_time DATETIME DEFAULT GETDATE(),
  CONSTRAINT PK_patient PRIMARY KEY (p_atient_id)
);

-- 添加注释
EXEC sp_addextendedproperty 
    @name = N'MS_Description', @value = N'病人身份证号',
    @level0type = N'SCHEMA', @level0name = N'dbo',
    @level1type = N'TABLE', @level1name = N'patient_3123004721_袁子轩',
    @level2type = N'COLUMN', @level2name = N'p_atient_id';

EXEC sp_addextendedproperty 
    @name = N'MS_Description', @value = N'病人信息表_3123004721_袁子轩',
    @level0type = N'SCHEMA', @level0name = N'dbo',
    @level1type = N'TABLE', @level1name = N'patient_3123004721_袁子轩';
```
...
```sql
-- SQL Server建表语句
IF OBJECT_ID('drugs_3123004721_袁子轩', 'U') IS NOT NULL
    DROP TABLE drugs_3123004721_袁子轩;
...
CREATE TABLE drugs_3123004721_袁子轩 (
  ...
  drug_storage VARCHAR(50) NOT NULL,
  ...
  CONSTRAINT CHK_drug_quantity CHECK (drug_quantity >= 0)
);

-- 添加注释
EXEC sp_addextendedproperty 
    @name = N'MS_Description', @value = N'药品信息表_3123004721_袁子轩',
    @level0type = N'SCHEMA', @level0name = N'dbo',
    @level1type = N'TABLE', @level1name = N'drugs_3123004721_袁子轩';
```
...
- ✅ 成功创建触发器：recipel_update（药品出库）
- ✅ 成功创建存储过程：count_people_date（统计科室问诊人数）
- ✅ 所有表结构符合SQL Server规范
...
对于本次医院信息系统的数据库设计，虽然实际采用了手工SQL编写+Python Flask开发的方式，但遵循了Power Designer的设计理念：

1. 先设计E-R模型（等同CDM）
2. 转换为关系模式（等同LDM）
3. 进行规范化分析和优化
4. 生成SQL Server建表脚本（等同PDM）
5. 编写详细的设计文档
