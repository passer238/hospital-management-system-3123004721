...
本系统开发所需软件均可免费获取或使用教育版本，硬件方面只需普通PC即可运行，无需高额的硬件投资。系统上线后可显著提升门诊工作效率，减少人力成本，具有良好的经济效益。
...
```mermaid
erDiagram
    PATIENT ||--o{ REGISTRATION : "登记"
    DOCTOR ||--o{ REGISTRATION : "接诊"
    REGISTRATION ||--o| PRESCRIPTION : "关联"
    DOCTOR ||--o{ PRESCRIPTION : "开具"
    PATIENT ||--o{ PRESCRIPTION : "接收"
    PRESCRIPTION ||--|{ PRESCRIPTION_DRUG : "包含"
    DRUG ||--o{ PRESCRIPTION_DRUG : "组成"
    DRUG ||--o{ CHARGE : "收费项"
    PATIENT ||--o{ CHARGE : "消费"
    CHARGE ||--o{ PAYMENT : "支付"
    PAYMENT ||--|| PICKUP : "生成"
    ...
    DRUG {
        ...
        nvarchar drug_name
        decimal drug_price
        bigint drug_quantity
        nvarchar drug_storage
        datetime drug_date
        datetime usefull_life
    }
